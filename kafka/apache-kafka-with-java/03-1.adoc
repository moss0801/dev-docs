...
* -1 (or all): 토픽의 min.insync.replicas 개수에 해당하는 리더 파티션과 팔로워 파티션에 데이터가 저장되면 성공으로 판단
...
메시지를 바로바로 브로커로 전송할 수 있는 상황에서는 배치가 쌓일 때까지 기다리지 않고 전송 +
기본값이 0이기 때문에 전송 가능하다면 바로 전송되어 레코드들이 배치로 묶이지 않음. 문서상 수정 예는 5 (5ms) +
```
...
* retries : value > 0 (기본값: Integer.MAX_VALUE)
* acks : all (기본값: all)
...
비동기로 결과를 기다리는 동안 다음으로 보낼 데이터의 전송이 성공하고 앞서 보낸 데이터의 결과가 실패할 경우 재전송으로 인해 데이터 순서가 역정될 수 있음 +
